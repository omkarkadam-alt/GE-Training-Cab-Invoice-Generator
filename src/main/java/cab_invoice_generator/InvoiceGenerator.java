...
public class InvoiceGenerator {
    ...
    public double calculatePremiumFare(double dist, double time) {

        return Math.max(PREMIUM_MINIMUM_FARE, (PREMIUM_COST_PER_KM * dist) + (PREMIUM_COST_PER_MINUTE * time));
    }

    public double calculateFareForMultipleRides(Ride[] rides) {
        double totalFare = 0;
        for (Ride ride : rides) {
            if(ride.getType() == "normal")
